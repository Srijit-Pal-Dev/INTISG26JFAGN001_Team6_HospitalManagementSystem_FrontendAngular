...
import { AppointmentDTO, AppointmentStatus, PrescriptionResponse } from '../../../../../core/models/index';
...
export class DoctorAppointmentsComponent implements OnInit {
    ...
    prescriptionAppointmentIds = signal<Set<number>>(new Set());
    ...
    hasPrescription(appointmentId: number): boolean {
        return this.prescriptionAppointmentIds().has(appointmentId);
    }

    viewPrescription(appointment: AppointmentDTO): void {
        this.isLoadingPrescription.set(true);
        this.showPrescriptionModal.set(true);
        this.prescriptionService.getPrescriptionByAppointment(appointment.id).subscribe({
            next: (p) => {
